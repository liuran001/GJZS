Configuration=2021083105
Magisk_Warehouse_version=2021083102
App_Store_version=2021083101
...
time='2021年8月15日'
    [[ $Choice = 1 ]] && Download -gh 'topjohnwu/Magisk/releases/download/v23.0/Magisk-v23.0.apk' "$1.apk" 6874374 ea450d98c25b4e09bb353e5bef8ca8a0 "$1.apk"
;;
...
id='He_zheng'
name='亮屏满血特别版'
version='1.3(21.08.31)'
versionCode=1
author='酷安ID:鹤征'
description='适合小米全机型(10,11,k30,k20,k40测试通过)，温度封顶49度及以下，异常高温自动断电(50度)。删除温控电流无法限制！要是没温控建议刷完整包!此版为特别版，如果需要k30系，推荐刷1.2版本！'
time='2021年8月31日'
    if [[ $lpkc_module -eq 1 ]]; then
    [[ $Choice = 1 ]] && Download -file "Modules/V1.2xmlpkc.zip" "$1.zip" 11429 3815258d2eee8cade4d015ba36019863 "$1.zip"
    else
    [[ $Choice = 1 ]] && Download -file "Modules/V1.3xmlpkc.zip" "$1.zip" 11415 d9373b19219f10f139b0a2c36b0748fc "$1.zip"
    fi
;;
...
time='2021年8月29日'
    [[ $Choice = 1 ]] && Download -gh "RikkaApps/Sui/releases/download/v12.1.4/sui-v12.1.4-release.zip" "$1.zip" 1081430 f736711f3fc24dcdf7ee1e6e331a68b0 "$1.zip"
;;
...
Install_Applet)
    name=Applet
    versionCode=75
    Install_Applet2 -file "Other/Applet/Applet-75.zip" "$name.zip" 7769031 742b1ac438f07659618ebc94181d354f "$name" $versionCode
;;
