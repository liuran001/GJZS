    ...
    Extracting_files() {
        ...
        [[ $RECOVERYMODE = 1 ]] && export RECOVERYMODE=true || export RECOVERYMODE=false
            ...
            if [[ $user_version -lt $versionCode || ! -f $APK ]]; then
                ...
                unzip -o "$Download_File" lib/x86/libbusybox.so lib/armeabi-v7a/libbusybox.so lib/x86_64/libbusybox.so lib/arm64-v8a/libbusybox.so -d $TMPDIR &>/dev/null
                chmod -R 755 $TMPDIR/lib
                api_level_arch_detect
                ui_print "- 设备架构：$ABILONG"
                mv -f $TMPDIR/lib/$ABILONG/libbusybox.so $BBBIN
                $BBBIN >/dev/null 2>&1 || abort "- 不支持的架构$ARCH"
                rm -rf $TMPDIR/lib
                mkdir -p $INSTALLER
