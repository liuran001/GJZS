...
<group title="其它">
    <switch interruptible="false">
        <title>在线/离线版切换</title>
        <desc>关闭此开关将不会连接服务器，但也会缺失所有需要联网才能使用的功能，切换后会强制重启搞机助手【需要使用3.2以上版本才会生效】</desc>
        <get>. $PREFIX/kr-script/switchs/Change_Local_get.sh</get>
        <set>. $PREFIX/kr-script/switchs/Change_Local_set.sh</set>
    </switch>
...
cat <<Han
    <text>
        <slice size="15" color="#FF0F9D58">已检测到您的系统缺少curl命令文件，连接服务器失败！【有ROOT的用户请打开MT管理器侧边栏终端模拟器，然后重启助手】</slice>
    </text>
<group>
    <text>
        <title>读取云端页面异常</title>
